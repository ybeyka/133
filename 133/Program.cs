using System;
public class RadioOffException : Exception
{
    public RadioOffException(string message) : base(message)
    {
    }
}
...
{
    private const int SpeedLimit = 150;
    private int _shvudkist;

   

    public int Shvudkist
    {
        get
        {
            return _shvudkist;
        ...
        {
            if (value > SpeedLimit)
                throw new SpeedLimitExceededException("Швидкість перевищує максимально допустиму!");
            _shvudkist = value;
        }
    }
    ...
    {
        if (!IsOn) throw new RadioOffException("Радіо вимкнено! Спочатку ввімкніть радіо.");
        Station = station;
    }
...
class Program
{
    static void Main(string[] args)
    {
        ...
        while (true)
        {
            ...
            {
                Console.WriteLine("Ох тяжело идет коняка ");
            }
            Console.WriteLine();
        }
    }
